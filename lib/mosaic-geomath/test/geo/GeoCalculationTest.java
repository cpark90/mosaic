...
//        assertTrue(azimuthError.meanError() < 1d);
    ...
    public void spheroidGeoCalculator_shortDistance() throws Exception {
        ...
        GeoProjection.getInstance().setGeoCalculator(new HarvesineGeoCalculator());
        massTest(distanceError, azimuthError, roundTripError, "lib/mosaic-geomath/test/resources/distance_test_values_short.csv");
...
//        assertTrue(azimuthError.meanError() < 1d);
    ...
    public void utmGeoCalculator_shortDistance_Berlin() throws Exception {
        ...
        GeoProjection.getInstance().setGeoCalculator(new UtmGeoCalculator());
        massTest(distanceError, azimuthError, roundTripError, "lib/mosaic-geomath/test/resources/distance_test_values_short_berlin.csv");
...
//        assertTrue(azimuthError.meanError() < 1d);
    ...
    private void massTest(MeanErrorAggregator distanceError, MeanErrorAggregator azimuthError, MeanErrorAggregator roundTripError) throws IOException {
        massTest(distanceError, azimuthError, roundTripError, "lib/mosaic-geomath/test/resources/distance_test_values.csv");
    }
    ...
    public void performanceTest() throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream("/lib/mosaic-geomath/test/resources/distance_test_values.csv")))) {
            reader.lines().forEach(line -> {
                        String[] items = line.split(";");
